...
class Permissions(commands.Cog):
    ...
    @commands.has_role("name") # @commands.has_role(id) Позволяет выполнить команду если у человека есть заданная вами роль, название в кавычках либо айди без кавычек (Можно и то и то сразу)
    async def user_has_role(self, ctx):
        await ctx.send("Права есть!")
        
    @commands.command(name="Наличие_ролей_у_участника")
    @commands.has_any_role("one","two","three") # @commands.bot_has_role(id1, id2, id3) #Позволяет выполнить команду если у человека есть одна из есть заданныъ вами ролей, названия в кавычках либо айди без кавычек (Можно и то и то сразу)
    async def user_has_any_role(self, ctx):
        await ctx.send("Права есть!")
      
    @commands.command(name="Наличие_роли_у_бота")
    @commands.bot_has_role("name") # @commands.bot_has_role(id) #Позволяет выполнить команду если у бота есть заданная вами роль, название в кавычках либо айди без кавычек (Можно и то и то сразу)
    async def bot_has_role(self, ctx):
        await ctx.send("Права есть!")
        
    @commands.command(name="Наличие_ролей_у_бота")
    @commands.bot_has_any_role("one","two","three") # @commands.bot_has_any_role(id1, id2, id3) #Позволяет выполнить команду если у бота есть заданная вами роль, название в кавычках либо айди без кавычек (Можно и то и то сразу)
